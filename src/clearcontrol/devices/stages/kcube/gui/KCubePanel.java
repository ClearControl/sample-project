 ...
 */
public class KCubePanel extends CustomGridPane
{
  KCubeDevice mKCubeDevice;

  public KCubePanel(KCubeDevice pKCubeDevice)
  {
    //super(pKCubeDevice);
...

    {
      ...
      lButton.setOnAction(new EventHandler<ActionEvent>()
      {
        @Override public void handle(ActionEvent event)
        {
          mKCubeDevice.moveBy(lStepVariable.get());
        }
      });
      add(lButton, 1, lRow);
      lRow++;
...

    {
      ...
      lButton.setOnAction(new EventHandler<ActionEvent>()
      {
        @Override public void handle(ActionEvent event)
        {
          mKCubeDevice.moveBy(-1 * lStepVariable.get());
        }
      });
      add(lButton, 1, lRow);
      lRow++;
